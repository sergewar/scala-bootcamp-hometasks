package com.sss.scalabootcamp.hometasks.task6
...
object ImplicitsHomework {
  ...
  object SuperVipCollections4s {
    ...
    final class MutableBoundedCache[K: GetSizeScore, V: GetSizeScore](maxSizeScore: SizeScore) {
      ...
      def put(key: K, value: V): Unit = {
        val currentDataSizeScore = map.map{case (k, v) => k.sizeScore + v.sizeScore}.sum
        val addingDataSizeScore = key.sizeScore + value.sizeScore
        if (currentDataSizeScore + addingDataSizeScore <= maxSizeScore) {
          map.put(key, value)
        } else {
          if (map.nonEmpty) {
    ...
    object instances {
      ...
      /*- String */
      implicit val stringGetSizeScore: GetSizeScore[String] = string => objectHeaderSize + string.length * 2
      ...
      implicit def iterableGetSizeScore[T: GetSizeScore, I[_]: Iterate]: GetSizeScore[I[T]] = (collection: I[T]) => {
        objectHeaderSize + implicitly[Iterate[I]].iterator(collection).map(_.sizeScore).sum
      }

      implicit def iterable2GetSizeScore[T1: GetSizeScore, T2: GetSizeScore, I[_, _]: Iterate2]: GetSizeScore[I[T1, T2]] = (collection: I[T1, T2]) => {
        val iterator1 = implicitly[Iterate2[I]].iterator1(collection)
        val iterator2 = implicitly[Iterate2[I]].iterator2(collection)
        objectHeaderSize + iterator1.map(_.sizeScore).sum + iterator2.map(_.sizeScore).sum
      }
      /*
  ...
  object MyTwitter {
    ...
    implicit def fbiNoteGetSizeScore: GetSizeScore[FbiNote] = (note: FbiNote) =>
